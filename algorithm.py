...
replace_population_dynamic, replace_population_clustering, replace_population_sharing_separate, replace_population_dynamic_separate,\
replace_population_dynamic_elitism, sgd_steps
...
def EvolveSGD_dynamic_elitism(num_generations, population_size, dim, n_niches, niche_radius, obj_f, network, data_x, data_y, criterion, n_epochs, sgd_lr, batch_size):
    # Initialization
    population = initialize_population(population_size, dim)
    generation_list = []
    loss_list = []
    for generation in range(num_generations):
        ...
        # Replace Old Population
        population, loss = replace_population_dynamic_elitism(population, offspring, population_size, n_niches, niche_radius, obj_f)
        best_loss = min(loss)
