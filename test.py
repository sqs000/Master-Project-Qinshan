...
if __name__ == "__main__":
    ...
    for seed in range(6, 106):
        ...
        f_3_d_2_generator = data_generator(suite_name="bbob", function=7, dimension=2, instance=1)
        ...
        GA_SGD_loss = min(loss_values)
        GA_SGD_losses.append(GA_SGD_loss)
        print(GA_SGD_loss)
        ...
        sgd_loss_values = [objective_function(x) for x in sgd_population]
        SGD_loss = min(sgd_loss_values)
        SGD_losses.append(SGD_loss)
        print(SGD_loss)
        print()

        if GA_SGD_loss < SGD_loss:
            GA_SGD_better_count += 1

    print(f"GA_SGD_better_count: {GA_SGD_better_count}.")
